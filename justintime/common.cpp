...
void ReadDevURandom256( uint8_t *pDest, uint32_t destLen )
{
        FILE *pFile;
	...
	if ( destLen < 32 )
        ...
        if ( fread( pDest, sizeof(uint8_t), 32, pFile ) != 32 )
        {
                printf( "Couldn't get enough entropy, seed used\n" );
...
uint32_t DoCRC32( uint32_t crc, const uint8_t *buf, uint32_t size )
{
	const uint8_t *p;
...
void FreeMessageWrapper( void *pData )
{
	if ( pData )
		delete [] pData;
}
